...
Paper nav must land on `#view-paper` tables with `#home` hidden. Capture-phase Paper click calls `showPaper(true)` then `stopImmediatePropagation`, then `setView('paper')` (Breakout passes it through — it no longer `hideNativeViews` on non-BB views). Experimental and Breakout `kindOf` return `""` for Paper (not `"other"`); Experimental `show(false)` never unhides `#home` while `data-fd-paper` / `#view-paper.fd-paper-on`. Recopy `paper_trade.py`, `s_score.py`, and `breakout.py`.
