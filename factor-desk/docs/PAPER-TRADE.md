...
Recopy `breakout.py` too. Live `breakout.js` `setView` used to `show("")` for every non-BB view (including Paper) and the capture `kindOf(PaperBtn)` returned `"other"` — both bounce Paper → Home. Durable embed: `kindOf` returns `""` for `paper` / `experimental` / `home` / `mom-*` (and `#fd-nav-paper`); `setView("paper"|"experimental")` calls the live `orig` only; `show("")` runs only when leaving Breakout/Breakdown panes and must not hide Paper or unhide Home. Capture never handles `#fd-nav-paper`. Live path is `C:\Users\MLP\Desktop\factorbook.html` (sibling of the pack).
