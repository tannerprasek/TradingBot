...
Experimental `kindOf` returns `""` for Paper / Breakout / Breakdown (marker `__FD_SS_IGNORE_FOREIGN_NAV__`) so `show(false)` cannot unhide `#home` after Paper already opened.
