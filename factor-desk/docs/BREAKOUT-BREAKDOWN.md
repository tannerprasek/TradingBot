...
- `breakout.ensure_embedded(html, ranked=None)` **does not skip the JS patch**. `#fd-breakout-js` is always replaced (version stamp `pr17-html-metrics-map`) so an old `__FD_BB_BOUND__` IIFE cannot keep the `cardHTML` fallback alive. `renderRow` never calls `cardHTML`.
