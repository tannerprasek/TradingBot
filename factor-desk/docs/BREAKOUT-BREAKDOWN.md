...
- Breakout/Breakdown cards are the **same** Momentum Up/Down cards. Live desk chrome is `<div class="card">` (not only `<article class="card">`). Mom panes only keep those cards while Mom is the active view, so membership is `#fd-bb-mom-cache` / `window.__FD_BB_MOM_CACHE__` first (DOM clone plus a serializable card object, snapshotted on Refresh, boot, and whenever Mom Up/Down paints — and not wiped when the live grids are emptied), then `#fd-bb-mom-full-db` (the full `cardHTML` object: metrics, pills, status, trend, blurb / flow line, scores, streak, residual), then the live panes, then `window.MOM`. `#fd-bb-mom-db` stays the thin streak row used for gates. A dense DOM node is cloned. A name with only a thin stub is painted with `cardHTML` on the full object, never the streak row. A missing gate field skips that card. Dispersion is read from the card object, then from the live stat chrome (`RS63` / `data-dispersion`), not only from a single `RS63 <number>` regex. When the Mom universe is non-empty and nothing passes, the empty line is `No early inflections (0 of N passed)`. The old `No early inflections this Refresh.` line is only for a page with no Mom data at all. An empty `#fd-breakout-db` does not blank a full Mom book. Do **not** use the skinny `__FD_BB_PORTABLE_FIX__` / `denseHTML` substitute.
