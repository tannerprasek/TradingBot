...
- Ranked JSON (`#fd-breakout-db`) carries top-level numeric `day` / `r20` / `rs63` / `atr_pct` **and** a nested `metrics` blob (`r20_pct` / `rs_63` / `atr_pct`) — the shape live Momentum `cardHTML` actually reads (`c.metrics.r20_pct`, not top-level `r20`). Returns are decimals (`0.1277` → desk `fmtPct` → `12.8%`). `atr_pct` is already percent points (~2–5); `fmtAtr` does **not** `*100` when `abs(value) >= 1`. Prefer `card.metrics` from the live book; else compute from `px_series` / `prices_long.csv` / live `px.by`.
...
- Breakout/Breakdown cards are **identical** to Momentum Up: `renderRow` finds the live MOM card by ticker (`__FD_FIND_CARD__` / `MOM.up|down|cards`) and calls **`cardHTML(card)`** — pills, status, streak, metrics, blurb, Buy/Sell. Skinny portable dense cards are a last-resort only if `cardHTML` is missing.
- **Card click opens the company name-drill** via `selectTicker(data-t)` — same as Mom Up. Capture-phase listener stops on `#breakout-grid article.card` / `#breakdown-grid article.card` and does **not** `setView("breakdown")`. View panes do **not** carry `data-view` (only nav buttons do), so live `closest("[data-view]")` cannot steal the click.
...
- `breakout.ensure_embedded(html, ranked=None)` **does not skip the JS patch**. `#fd-breakout-js` is always replaced (version stamp `pr17-mom-cardhtml`) so an old `__FD_BB_BOUND__` IIFE cannot keep a skinny dense fallback alive. `renderRow` looks up the MOM card and calls `cardHTML`.
