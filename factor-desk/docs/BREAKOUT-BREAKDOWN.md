...
- Breakout/Breakdown cards are the **same** Momentum Up/Down articles. `show` filters `#view-mom-up` / `#view-mom-down` (and `MOM.up|down|cards` when those panes have no articles) with the four gates and **clones** each match into the grid, so `data-t`, status, streak, digest, and filter attributes stay on the card. A name with no DOM article is rendered with the same `cardHTML(card)`. A missing gate field skips that card. The empty line is written only when that live universe has zero matches — an empty `#fd-breakout-db` does not blank a full Mom book. Do **not** use the skinny `__FD_BB_PORTABLE_FIX__` / `denseHTML` substitute.
...
- `breakout.ensure_embedded(html, ranked=None)` **does not skip the JS patch**. `#fd-breakout-js` is always replaced (version stamp `pr32-mom-filter`). `window.__FD_BB_SHOW__ =` is assigned whenever the Breakout/Breakdown nav is present (same binder rule as the live-desk nav check). `__FD_BB_BOUND__` is stamped only after that assignment. Paper clicks are ignored (`kindOf` → `""`, marker `__FD_BB_IGNORE_PAPER__`); `#view-paper` is not in BB `NATIVE_VIEWS`.
