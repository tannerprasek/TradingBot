...
Generic Buy/Sell on dense `cardHTML` cards. Paper only, 1 unit, localStorage `fd-paper-book`. Same-side click is a no-op + toast (does not flip). `paper_trade.ensure_embedded` on every HTML write. Recopy `paper_trade.py`; do not wholesale replace live `desk_dash.py`. See [PAPER-TRADE.md](PAPER-TRADE.md).
