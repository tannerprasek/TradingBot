...
4. `write_combined` embeds `#fd-search-book` (short symbol `t`, yellow `ticker`) and `#fd-symbol-bar`. The boot script merges those rows into `window.BOOK` / `window.NAMES` / `window.MOM.search` and filters them as you type (`window.__FD_SEARCH__`: exact symbol, prefix, first token, or company-name word). Typing `TSEM` / `TSEM US` in the symbol bar or in `#search-pane` lists the name and calls `selectTicker`. Hits are also painted into `#search-pane` (`#fd-search-pane-hits`), because the live MOM card list does not contain a name that only exists in the factor files. Add posts to `POST /api/add`. `aapl us equity` and `AAPL US EQUITY` store as `AAPL US Equity`. A symbol that merely ends in `PFD` / `CORP` / `INDEX` is not treated as a yellow key. Fat chrome is patched, not replaced. The card universe also reads `v0_residuals.csv` / `residual_panel.json` and `ticker_names.json`.
