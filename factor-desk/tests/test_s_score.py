...
class ForeignNavIgnoreTests(unittest.TestCase):
    def test_kindof_returns_empty_for_paper_and_bb(self) -> None:
        js = ss.strip_js()
        self.assertIn("__FD_SS_IGNORE_FOREIGN_NAV__", js)
        self.assertIn("window.__FD_SS_IGNORE_FOREIGN_NAV__ = true", js)
        kind = js.split("function kindOf")[1].split("window.__FD_SS_KIND_OF__")[0]
        self.assertIn('view === "paper"', kind)
        self.assertIn('view === "breakout"', kind)
        self.assertIn('view === "breakdown"', kind)
        paper_return = kind.split('view === "paper"')[1].split("if (btn.id")[0]
        ...
        self.assertIn("return orig.apply(this, arguments)", bridge)
