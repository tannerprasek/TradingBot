...
class MaTrendTests(unittest.TestCase):
    ...
    def test_overlay_wraps_paint_px_chart(self) -> None:
        ...
        self.assertNotIn("H=300", js)
        self.assertIn("xMidYMid meet", js)
        self.assertIn("stripRecentBand", js)
        self.assertIn("#c8b48a", js)
        ...
        self.assertIn(".px-chart-wrap", css)
        self.assertNotIn("height: 320px", css)
        draw = js.split("function drawTrend")[1].split("function wrapSelect")[0]
        self.assertIn("isPxHost(chart) || isDetailChart(chart)", draw)
        self.assertIn("restylePxChart(chart)", draw)
        self.assertIn("__FD_NO_PIXEL_MA__", draw)
        gate_at = draw.find("isDetailChart(chart)")
        pixel_at = draw.find("-p.y")
        flag_at = draw.find("__FD_NO_PIXEL_MA__")
        self.assertGreater(gate_at, 0)
        self.assertGreater(flag_at, 0)
        self.assertLess(flag_at, pixel_at)
        self.assertLess(gate_at, pixel_at)
        self.assertIn("__FD_NO_PIXEL_MA__", js)
        self.assertIn("__FD_CHART_BOX__", js)
