...
class MomAndMissingTests(unittest.TestCase):
    def test_weighted_mom_and_missing_names(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            mom_metric = next(m for m in card["metrics"] if m["key"] == "mom_score")
            self.assertIsNone(mom_metric["q1_gross"])
            self.assertIsNone(mom_metric["q5_gross"])
            self.assertLess(mom_metric["desk_n"], 5)
